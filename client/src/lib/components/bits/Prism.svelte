<!--
  ...
  Tailwind, only the tokens in client/src/tokens.css. Otherwise verbatim so it
  can be re-synced.
-->
<script lang="ts">
...
void main() {
  ...
  gl_FragColor = vec4(col, o.a);
		...
		return () => {
			...
			if (io) io.disconnect();
			if (gl.canvas.parentElement === containerRef) containerRef.removeChild(gl.canvas);
		};
	});
</script>

<div bind:this={containerRef} class="prism-canvas"></div>
