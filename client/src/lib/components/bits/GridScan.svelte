<!--
  ...
  Changes:
  - The face-tracking path is removed entirely, and with it the face-api.js
    dependency. It opened the FRONT camera on the landing screen and fetched
    model weights from a CDN at runtime. This app asks for the REAR camera
    later, with an explanation; a selfie prompt at the door would be refused by
    most people, and a refusal there poisons the request we actually need. The
    gyroscope handler the component already had drives the tilt instead, which
    is the right input on a phone anyway.
  - Tailwind utility classes replaced with scoped CSS — no Tailwind here.
-->
<script lang="ts">
...
`;
	...
	onMount(() => {
		...
		const MAX_SCANS = 8;
		// Upstream types this as Record<string, {value: unknown}>, which under this
		// project's noUncheckedIndexedAccess makes every lookup possibly-undefined.
		// The literal is fully known here, so inference gives concrete properties
		// and the strictness is kept rather than waived.
		const uniforms = {
...
<div bind:this={containerRef} class="grid-scan {className}">
</div>
