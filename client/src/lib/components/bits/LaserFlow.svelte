<!--
  ...
  Changes: the one Tailwind class swapped for scoped CSS — this app has no
  Tailwind, only the tokens in client/src/tokens.css.
-->
...
void mainImage(out vec4 fc,in vec2 frag){
  ...
  float browserFogIntensity=uFogIntensity;browserFogIntensity*=1.8;
...
<script lang="ts">
	...
	$effect(() => {
		...
		const uniforms = {
			...
			uColor: { value: new THREE.Vector3(c0.r, c0.g, c0.b) },
			uFade: { value: 0 }
		};
		...
		const animate = () => {
			...
			const alpha = mouseSmoothTime <= 0 ? 1 : 1 - Math.exp(-cdt / tau);
...
<div bind:this={mount} class="laser-flow {className}" {style}></div>
